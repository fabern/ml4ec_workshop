...
ddf <- read_csv("./data/FLX_CH-Dav_FLUXNET2015_FULLSET_DD_1997-2014_1-3.csv", na = "-9999") %>% 
...
eval_model(
  ...
  df_train = ddf_train, 
  df_test = ddf_test
  ) + labs(caption = "k-Nearest Neighbors with k=2")
```
```{r }
## with k = 30
eval_model(
  ...
  df_train = ddf_train, 
  df_test = ddf_test
  ) + labs(caption = "k-Nearest Neighbors with k=30")
```
...
- The visualisation of cross-validation results can be visualised with the `plot(model_object)` or `ggplot(model_object)`.
